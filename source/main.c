...
volatile uint8_t battery_status = 0; // 0: OK, 1: Low (<3V), 2: Critical (<2.8V)
#define VDD_RECOVERY_THRESHOLD 360   // (1.2V / 3.4V) * 1023
#define VDD_LOW_THRESHOLD      400   // (1.2V / 3V) * 1023
#define VDD_CRITICAL_THRESHOLD 430   // (1.2V / 2.8V) * 1023

#ifndef ADC_CTLR2_TSVREFE
#define ADC_CTLR2_TSVREFE      ((uint32_t)0x00800000)
#endif

#ifndef PWR_CTLR_CWUF
#define PWR_CTLR_CWUF          ((uint32_t)0x00000004)
#endif
#ifndef PWR_CTLR_CSBF
#define PWR_CTLR_CSBF          ((uint32_t)0x00000008)
#endif
...
int main(void){
    ...
    /* Wait for power rail and internal reference stabilisation on boot/reset */
    for(volatile uint32_t i=0; i<50000; i++);

    GPIO_LowPower_Init(); // Configure all unused logical pins to IPD to eliminate leakage
    LED_Init();
    TIM2_Init_Tick();
    ADC_Init_Vdd();
    Button_Init();
    /* Perform synchronous battery/solar check on cold boot */
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1, ENABLE); // Enable ADC clock
    ADC_Cmd(ADC1, ENABLE);
    ADC1->CTLR2 |= ADC_CTLR2_TSVREFE;
    for(volatile int i=0; i<150; i++); // Wait for reference to stabilize
    battery_status = Check_Battery();
    ADC1->CTLR2 &= ~ADC_CTLR2_TSVREFE; // Disable Vrefint channel
    ...
    while(1){
        ...
        if (batt_state == 0) {
            if (GetTick() - batt_timer > 10000) {
                ...
                ADC1->CTLR2 |= ADC_CTLR2_TSVREFE; // Enable Vrefint channel
                batt_timer = GetTick();
                batt_state = 1;
            }
        ...
        /* 5. Idle Standby Handling (When light is off) */
        /* Stay in Sleep mode (WFI) if charging to show the indicator blip */
        if (level_idx == 0 && !is_pressing && battery_status != 2 && charge_current_ma <= 2) {
            Enter_Deep_Sleep();
        }
...
void PWM_LED(u16 duty){
        ...
        if (duty == 0) {
            ...
            // Reconfigure PA1 as GPIO Output PP driven LOW to prevent floating gate
...
void TIM2_IRQHandler(void)
{
    if(TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
    {
        ...
        /* Mode LED (PC1) Control */
        if (is_pressing) {
            GPIO_WriteBit(GPIOC, GPIO_Pin_1, Bit_RESET);
        } else {
            /* Solar charging feedback on PC1:
             * 1 short blink for charging > 2mA, 2 blinks for > 15mA, 3 blinks for > 30mA , 4 > 50mA */
            if (charge_current_ma > 50) {
                uint32_t phase = ms_ticks % 3000;
                if (phase < 150 || (phase >= 300 && phase < 450) || (phase >= 600 && phase < 750) || (phase >= 900 && phase < 1050)) {
                    GPIO_WriteBit(GPIOC, GPIO_Pin_1, Bit_SET);
                } else {
                    GPIO_WriteBit(GPIOC, GPIO_Pin_1, Bit_RESET);
                }

            } else if (charge_current_ma > 30) {
                uint32_t phase = ms_ticks % 3000;
                if (phase < 150 || (phase >= 300 && phase < 450) || (phase >= 600 && phase < 750)) {
                    GPIO_WriteBit(GPIOC, GPIO_Pin_1, Bit_SET);
                } else {
                    GPIO_WriteBit(GPIOC, GPIO_Pin_1, Bit_RESET);
                }

            } else if (charge_current_ma > 15) {
		uint32_t phase = ms_ticks % 3000;
    ...
    if(EXTI_GetITStatus(EXTI_Line4) != RESET)
    {
        ...
        else // Button Released (Rising Edge)
        {
            if(is_pressing)
            {
                ...
                if(duration > 15) // Valid press (> 20ms)
                {
                    /* Smart Off: If ON for > 3s, next press turns it OFF */
...
void ADC_Init_Vdd(void) {
    ...
    /* PA2 as Analog Input for Solar Voltage Monitoring */
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1, DISABLE); // Disable ADC clock to save power
}

uint16_t Get_ADC_Val(uint8_t ch) {
    uint8_t sample_time = ADC_SampleTime_30Cycles;
    if (ch == ADC_Channel_Vrefint) {
        sample_time = ADC_SampleTime_241Cycles; // Give Vrefint more time to stabilise
    }
    ADC_RegularChannelConfig(ADC1, ch, 1, sample_time);
...
uint8_t Check_Battery(void) {
    ...
    for(int i=0; i<16; i++) { // Increased oversampling for 1 ohm stability
        vref_sum += Get_ADC_Val(ADC_Channel_Vrefint);
        va2_sum += Get_ADC_Val(ADC_Channel_0); 
    }
    uint16_t vref_avg = vref_sum >> 4; // Divide by 16
    uint16_t va2_avg = va2_sum >> 4;   // Divide by 16
...
void Enter_Deep_Sleep(void) {
    ...
    /* Configure EXTI Line 9 for AWU Event Wakeup */
    EXTI_InitTypeDef EXTI_InitStructure = {0};
    EXTI_InitStructure.EXTI_Line = EXTI_Line9;
    EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Event;
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling; // Use falling edge to match the official example
    EXTI_InitStructure.EXTI_LineCmd = ENABLE;
    EXTI_Init(&EXTI_InitStructure);

    /* Enable LSI clock for AWU */
    RCC_LSICmd(ENABLE);
    while(RCC_GetFlagStatus(RCC_FLAG_LSIRDY) == RESET);

    /* Configure Auto Wakeup (AWU) to wake up every ~1.5s:
     * LSI (128kHz) / 10240 = 12.5 Hz clock.
     * AWU counts down from 63 to 44 (63 - 44 = 19 cycles).
     * 19 cycles / 12.5 Hz = 1.52 seconds wakeup interval. */
    PWR_AWU_SetPrescaler(PWR_AWU_Prescaler_10240);
    PWR_AWU_SetWindowValue(44);
    PWR_AutoWakeUpCmd(ENABLE);

    /* Clear pending wakeup event flag */
    EXTI_ClearITPendingBit(EXTI_Line9);

    /* Disable SDI (Single-Wire Debug) to save power in Standby.
     * SWIO pin (PD1) is released to GPIO. Configure it as Pull-Down to prevent floating. */
    ...
    /* Enter STANDBY mode using WFE (required for AWU Event Wakeup) */
    PWR_EnterSTANDBYMode(PWR_STANDBYEntry_WFE);
    
    /* Wake up: Hardware defaults to 24MHz HSI. Must restart PLL for 48MHz */
    SystemInit(); 

    /* Re-enable SDI (Single-Wire Debug) on wakeup so we can still debug/flash */
    GPIO_PinRemapConfig(GPIO_Remap_SDI_Disable, DISABLE);

    /* Wait for power rail and internal reference stabilisation on wakeup.
       Since VDD was held stable during standby, we only need a tiny delay (e.g. 500 iterations)
       instead of 50000, which saves ~25ms of active current on every wakeup. */
    for(volatile uint32_t i=0; i<500; i++);

    /* Re-initialize ADC after standby wakeup */
    ADC_Init_Vdd();

    /* Perform synchronous battery/solar check on wakeup */
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1, ENABLE); // Enable ADC clock
    ADC_Cmd(ADC1, ENABLE);
    ADC1->CTLR2 |= ADC_CTLR2_TSVREFE;
    for(volatile int i=0; i<150; i++); // Wait for reference to stabilize
